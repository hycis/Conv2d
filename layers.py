...
class My_MLP(MLP):
    ...
    def get_monitoring_channels(self, data):
        ...
        for layer in self.layers:
            ...
            train_ch = layer.get_train_monitoring_channels()
            for key in train_ch:
                rval['train'+layer.layer_name+'_'+key] = train_ch[key]
            train_state = layer.fprop(train_state)
            train_args = [train_state]
            if layer is self.layers[-1]:
                train_args.append(Y)
            train_ch = layer.get_train_monitoring_channels_from_state(*train_args)
            if not isinstance(train_ch, OrderedDict):
                raise TypeError(str((type(train_ch), layer.layer_name)))
            for key in train_ch:
                rval['train'+layer.layer_name+'+'+key ] = train_ch[key]
...
#     
    ...
    def get_train_monitoring_channels(self):
        pass
    def get_train_monitoring_channels_from_state(self, target=None):
        pass
...
class My_Softmax(Softmax):
    ...
    def get_train_monitoring_channels(self):
        return self.get_monitoring_channels()
    
    def get_train_monitoring_channels_from_state(self):
